...
const findOrCreateCart = async (req) => {
  ...
  if (!filter) {
    res.status(400);
    throw new Error(
      "Missing guest identifier. Send x-guest-id header for guest carts.",
    );
  }
...
exports.getCart = asyncHandler(async (req, res) => {
  ...
  if (!filter)
    return res
      .status(200)
      .json({ success: true, cart: { items: [], subtotal: 0 } });
  ...
  res
    ...
    .json({
      success: true,
      cart: { ...cart.toObject(), subtotal: cart.subtotal() },
    });
});
...
exports.mergeGuestCart = asyncHandler(async (req, res) => {
  ...
  res
    ...
    .json({
      success: true,
      cart: { ...userCart.toObject(), subtotal: userCart.subtotal() },
    });
});
