...
const Orders = () => {
  ...
  useEffect(() => {
    api.get('/users/orders').then(({ data }) => setOrders(data.orders)).catch(() => setOrders([]));
  }, []);
  ...
  return (
    <div className="container-gully py-10">
      ...
      <div className="divide-y divide-ink/10 border-y border-ink/10">
        {orders.map((o) => (
          <Link key={o._id} to={`/orders/${o._id}`} className="flex items-center justify-between py-5 hover:bg-white/60 transition-colors px-2 -mx-2">
            <div>
              <p className="font-mono font-semibold text-sm">{o.orderNumber}</p>
              <p className="text-xs text-ink-soft mt-1">{new Date(o.createdAt).toLocaleDateString('en-PK', { year: 'numeric', month: 'long', day: 'numeric' })}</p>
            </div>
            <div className="flex items-center gap-4">
              <span className="font-semibold text-sm">{formatPrice(o.pricing.total)}</span>
              <span className={`text-xs font-mono uppercase tracking-wide px-3 py-1 rounded-full ${STATUS_COLORS[o.status]}`}>{o.status}</span>
            </div>
          </Link>
        ))}
      </div>
