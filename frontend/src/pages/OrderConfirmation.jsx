...
const OrderConfirmation = () => {
  ...
  if (!order) {
    return <div className="container-gully py-24 text-center text-ink-soft">Loading your order...</div>;
  }

  return (
    <div className="container-gully py-16 max-w-2xl">
      <div className="text-center mb-10">
        <CheckCircle2 size={48} className="text-pitch mx-auto mb-4" />
        <h1 className="font-display text-3xl font-bold mb-2">Thank you, {order.customer.fullName.split(' ')[0]}!</h1>
        <p className="text-ink-soft">Your order has been placed. A confirmation email is on its way.</p>
      </div>
      ...
      <div className="bg-white border border-ink/10 rounded-sm p-6 mb-8 text-sm">
        ...
        <p className="text-ink-soft uppercase">{order.paymentMethod.replace('_', ' ')}</p>
      </div>

      <div className="flex flex-col sm:flex-row gap-3 justify-center">
        <Link to="/shop" className="btn-secondary">Continue Shopping</Link>
        <Link to="/order-lookup" className="btn-primary">Track This Order</Link>
      </div>
    </div>
  );
};
