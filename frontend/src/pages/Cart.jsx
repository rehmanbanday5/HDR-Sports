...
const Cart = () => {
  ...
  if (loading) {
    return (
      <div className="container-gully py-16">
    ...
    <div className="container-gully py-10">
      ...
      <div className="grid lg:grid-cols-[1fr_360px] gap-10">
          ...
          {cart.items.map((item) => (
            <div key={item._id} className="flex gap-4 py-5">
              ...
              <div className="flex-1 min-w-0">
                <p className="font-display font-semibold leading-snug mb-1">{item.name}</p>
        ...
        <div className="bg-white border border-ink/10 rounded-sm p-6 h-fit sticky top-32">
          ...
          <p className="text-xs text-ink-soft mb-4">Shipping and taxes calculated at checkout.</p>
          <button onClick={() => navigate('/checkout')} className="btn-primary w-full">
            Proceed to Checkout <ArrowRight size={16} />
          </button>
          <Link to="/shop" className="block text-center text-sm text-pitch font-semibold mt-4 hover:underline">
            Continue Shopping
          </Link>
        </div>
      </div>
