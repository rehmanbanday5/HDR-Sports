...
const ProductDetail = () => {
  ...
  if (!product) {
    return (
      <div className="container-gully py-16 grid lg:grid-cols-2 gap-12">
  ...
  return (
    <div className="container-gully py-10">
      <div className="grid lg:grid-cols-2 gap-12">
        ...
        <div>
          {product.category?.name && (
            <Link to={`/shop?categorySlug=${product.category.slug}`} className="eyebrow hover:underline">
              {product.category.name}
            </Link>
          )}
          <h1 className="font-display text-3xl font-bold mt-2 mb-2">{product.name}</h1>
          ...
          <div className="flex items-baseline gap-3 mb-6">
            ...
            {originalPriceVal && <span className="text-lg text-ink-soft line-through">{formatPrice(originalPriceVal)}</span>}
          </div>

          {product.shortDescription && <p className="text-ink-soft leading-relaxed mb-6">{product.shortDescription}</p>}

          {/* Variant selectors */}
          {Object.entries(attributeOptions).map(([key, values]) => (
            <div key={key} className="mb-5">
              <p className="text-xs font-mono uppercase tracking-widest text-ink-soft mb-2">{key}</p>
                ...
                {values.map((val) => (
                  <button
                    ...
                    className={`px-4 py-2 text-sm border rounded-sm transition-colors ${
                      selectedAttrs[key] === val ? 'border-pitch bg-pitch text-chalk' : 'border-ink/20 hover:border-ink/40'
                    }`}
                  >
                    {val}
                  </button>
          ...
          <div className="flex items-center gap-2 mb-6 text-sm">
            {inStock ? (
              <><CheckCircle2 size={16} className="text-pitch" /> <span>In stock{stock <= 5 ? ` — only ${stock} left` : ''}</span></>
            ) : (
              <><XCircle size={16} className="text-leather" /> <span>Out of stock</span></>
            )}
          </div>
          ...
          <div className="flex items-center gap-4 mb-6">
            <div className="flex items-center border border-ink/20 rounded-sm">
              <button onClick={() => setQuantity((q) => Math.max(1, q - 1))} className="p-3 hover:bg-ink/5"><Minus size={14} /></button>
              <span className="w-10 text-center text-sm font-semibold">{quantity}</span>
              <button onClick={() => setQuantity((q) => Math.min(stock || 99, q + 1))} className="p-3 hover:bg-ink/5"><Plus size={14} /></button>
            </div>
          </div>

          <div className="flex flex-col sm:flex-row gap-3 mb-8">
            <button onClick={() => handleAddToCart(false)} disabled={!inStock} className="btn-secondary flex-1 disabled:opacity-40 disabled:cursor-not-allowed">
              <ShoppingBag size={18} /> Add to Cart
            </button>
            <button onClick={() => handleAddToCart(true)} disabled={!inStock} className="btn-accent flex-1 disabled:opacity-40 disabled:cursor-not-allowed">
              <Zap size={18} /> Buy Now
            </button>
          </div>
      ...
      <section className="border-t border-ink/10 mt-16 pt-10 max-w-2xl">
        <h2 className="font-display text-2xl font-semibold mb-6">Reviews ({product.reviews?.length || 0})</h2>
        ...
        {product.reviews?.length > 0 ? (
          ...
          <p className="text-sm text-ink-soft mb-10">No reviews yet. Be the first to share your thoughts.</p>
        )}

        {user ? (
          <form onSubmit={submitReview} className="border border-ink/10 rounded-sm p-5">
          ...
          <p className="text-sm text-ink-soft">
            <Link to="/login" className="text-pitch font-semibold hover:underline">Log in</Link> to leave a review.
          </p>
        )}
      </section>
