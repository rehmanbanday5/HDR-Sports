...
const AdminProductForm = () => {
        ...
        if (product.variants?.length) {
          setVariants(
            product.variants.map((v) => ({
              key: v._id,
              attrs: Object.entries(v.attributes || {}).map(([k, val]) => ({
                k,
                v: val,
              })),
              price: v.price,
  ...
  const removeVariantAttr = (idx, attrIdx) =>
    setVariants((vs) =>
      vs.map((v, i) =>
        i !== idx
          ? v
          : {
              ...v,
              attrs: v.attrs.filter((_, ai) => ai !== attrIdx),
            },
      ),
    );
 ...
 const handleSubmit = async (e) => {
   ...
   try {
       ...
       const variantPayload = variants
         ...
         .map((v) => ({
           attributes: Object.fromEntries(
             v.attrs.filter((a) => a.k).map((a) => [a.k, a.v]),
           ),
     ...
     // Product specifications
     const specPayload = Object.fromEntries(
       specs.filter((s) => s.k).map((s) => [s.k, s.v]),
     );
      ...
      <form onSubmit={handleSubmit} className="space-y-8">
        <section className="bg-white border border-ink/10 rounded-sm p-6 space-y-4">
          ...
          <div className="grid sm:grid-cols-2 gap-4">
            <select
              ...
              value={form.category}
              onChange={handleChange}
              className="border border-ink/20 rounded-sm px-4 py-2.5 text-sm bg-white"
            >
          ...
          {!form.hasVariants ? (
            ...
            <div className="space-y-4">
              {variants.map((v, idx) => (
                <div
                  key={v.key}
                  className="border border-ink/15 rounded-sm p-4 space-y-3"
                >
                    ...
                    {v.attrs.map((a, ai) => (
                      <div key={ai} className="flex gap-2">
                        <input
                          value={a.k}
                          onChange={(e) =>
                            updateVariantAttr(idx, ai, "k", e.target.value)
                          }
                          placeholder="Attribute (e.g. Size)"
                          className="flex-1 border border-ink/20 rounded-sm px-3 py-2 text-sm"
                        />

                        <input
                          value={a.v}
                          onChange={(e) =>
                            updateVariantAttr(idx, ai, "v", e.target.value)
                          }
                        ...
                        {v.attrs.length > 1 && (
                          <button
                            type="button"
                            onClick={() => removeVariantAttr(idx, ai)}
                            className="text-ink-soft hover:text-leather px-1"
                          >
                            <X size={14} />
                          </button>
                        )}
              ...
              <button
                type="button"
                onClick={addVariant}
                className="text-sm font-semibold text-pitch flex items-center gap-1 hover:underline"
              >
                <Plus size={14} />
                Add another variant
              </button>
          ...
          {specs.map((s, idx) => (
            <div key={s.key} className="flex gap-2">
              <input
                ...
                placeholder="Spec name (e.g. Willow Grade)"
                className="flex-1 border border-ink/20 rounded-sm px-3 py-2 text-sm"
              />

              <input
                value={s.v}
                onChange={(e) => updateSpec(idx, "v", e.target.value)}
                placeholder="Value"
                className="flex-1 border border-ink/20 rounded-sm px-3 py-2 text-sm"
              />
        ...
        <section className="bg-white border border-ink/10 rounded-sm p-6 space-y-4">
          ...
          <label
            ...
            className="inline-flex items-center gap-2 px-5 py-3 bg-ink text-chalk text-sm font-semibold rounded-sm cursor-pointer hover:bg-gold hover:text-ink transition-colors"
          >
            <Plus size={18} />
            Choose Images
          </label>
          ...
          <p className="text-xs text-ink-soft">
            You can select multiple product images together.
          </p>

          {/* Selected New Images Preview */}
          {newImages.length > 0 && (
            <div className="pt-2">
              <p className="text-sm font-medium mb-3">Selected Images</p>

              <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
                {newImages.map((file, index) => (
                  <div
                    key={`${file.name}-${index}`}
                    className="relative border border-ink/20 rounded-sm p-2 bg-white"
                  >
                    <img
                    ...
                    <button
                      ...
                      className="absolute top-2 right-2 h-7 w-7 rounded-full bg-ink text-chalk flex items-center justify-center hover:bg-leather transition-colors"
                    >
                      <X size={14} />
                    </button>
                  </div>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
                {existingImages.map((image) => (
                  ...
                  >
                    ...
                    <button
                      type="button"
                      onClick={() => removeExistingImage(id, image._id)}
                      className="absolute top-2 right-2 h-7 w-7 rounded-full bg-ink text-chalk flex items-center justify-center hover:bg-leather transition-colors"
                    >
                      <X size={14} />
                    </button>
