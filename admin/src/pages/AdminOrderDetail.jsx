...
const AdminOrderDetail = () => {
  ...
  const fetchOrder = () => {
    api.get(`/orders/${id}`).then((r) => setOrder(r.data.order)).catch(() => setOrder(false));
  };
  ...
  const updatePayment = async (newStatus) => {
    ...
    try {
      const { data } = await api.put(`/orders/${id}/payment-status`, { paymentStatus: newStatus });
      setOrder(data.order);
  ...
  return (
    <div className="max-w-3xl">
      <Link to="/admin/orders" className="text-sm text-pitch font-semibold hover:underline mb-4 inline-block">← Back to orders</Link>
      <div className="flex justify-between items-center mb-8">
        <h1 className="font-display text-3xl font-bold">{order.orderNumber}</h1>
        {order.isInternationalOrder && <span className="text-xs bg-willow/20 text-willow-dark px-2 py-1 rounded-full">International</span>}
      </div>
      ...
      <div className="grid md:grid-cols-2 gap-6 mb-6">
        <div className="bg-white border border-ink/10 rounded-sm p-6">
          ...
          <input
            ...
            placeholder="Note (e.g. tracking number, reason)"
            className="w-full border border-ink/20 rounded-sm px-3 py-2 text-sm"
          />
        </div>

        <div className="bg-white border border-ink/10 rounded-sm p-6">
          <h2 className="font-display font-semibold text-lg mb-4">Payment Status</h2>
          <div className="flex flex-wrap gap-2 mb-3">
          ...
          <p className="text-xs text-ink-soft">Method: <span className="uppercase">{order.paymentMethod}</span></p>
        </div>
      </div>
      ...
      <div className="bg-white border border-ink/10 rounded-sm p-6 mb-6">
            ...
            <div key={item._id} className="flex justify-between text-sm">
              <div>
                ...
                {item.attributes && Object.keys(item.attributes).length > 0 && (
                  <p className="text-xs text-ink-soft">{Object.entries(item.attributes).map(([k, v]) => `${k}: ${v}`).join(' · ')}</p>
                )}
              </div>
        ...
        <div className="border-t border-ink/10 pt-4 space-y-1.5 text-sm">
          <div className="flex justify-between"><span className="text-ink-soft">Subtotal</span><span>{formatPrice(order.pricing.subtotal)}</span></div>
          <div className="flex justify-between"><span className="text-ink-soft">Shipping ({order.shippingMethod.name})</span><span>{formatPrice(order.pricing.shippingCost)}</span></div>
          <div className="flex justify-between font-semibold text-base pt-2 border-t border-ink/10 mt-2"><span>Total</span><span>{formatPrice(order.pricing.total)}</span></div>
        </div>
      </div>
      ...
      {order.orderNotes && (
        <div className="bg-white border border-ink/10 rounded-sm p-6 mb-6 text-sm">
          <h2 className="font-display font-semibold text-lg mb-2">Order Notes</h2>
          <p className="text-ink-soft">{order.orderNotes}</p>
        </div>
      )}

      <div className="bg-white border border-ink/10 rounded-sm p-6 text-sm">
        <h2 className="font-display font-semibold text-lg mb-3">Status History</h2>
        <ul className="space-y-2">
          {order.statusHistory.map((h, i) => (
            <li key={i} className="flex justify-between text-xs text-ink-soft">
              <span className="capitalize font-medium text-ink">{h.status}{h.note ? ` — ${h.note}` : ''}</span>
